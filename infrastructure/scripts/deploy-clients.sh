#!/usr/bin/env bash
# =============================================================================
# deploy-clients.sh — PolizaLab CRM | Clients feature deployment
#
# Usage:
#   ./deploy-clients.sh [--env dev|stage|prod] [--region us-east-1]
#
# What this script does:
#   1. Creates/updates the Clients DynamoDB table via CloudFormation
#   2. Creates/updates the IAM role + inline policy via CloudFormation
#   3. Packages and deploys the client-handler Lambda function
...
#   - Lambda source code at: lambda/client-handler/handler.py
# =============================================================================
set -euo pipefail
...
LAMBDA_SRC_DIR="${PROJECT_ROOT}/lambda/client-handler"
...
# Derived names
# ---------------------------------------------------------------------------
FUNCTION_NAME="polizalab-client-handler-${ENV}"
DYNAMO_STACK="polizalab-clients-table-${ENV}"
IAM_STACK="polizalab-client-handler-role-${ENV}"
ARTIFACTS_BUCKET="polizalab-lambda-artifacts-${ENV}"
ARTIFACTS_KEY="client-handler/${FUNCTION_NAME}.zip"
DYNAMO_TABLE_NAME="Clients-${ENV}"
ROLE_NAME="polizalab-client-handler-role-${ENV}"

DYNAMO_CF="${INFRA_DIR}/dynamodb/clients-table.json"
IAM_CF="${INFRA_DIR}/iam/client-handler-policy.json"

TAG_LIST="Project=PolizaLab Feature=Clients Environment=${ENV}"
...
upload_lambda_zip() {
  ...
  aws s3 cp "$zip_path" "s3://${ARTIFACTS_BUCKET}/${ARTIFACTS_KEY}" \
    --region "$REGION" \
    --metadata "Environment=${ENV},Feature=Clients,Project=PolizaLab"
...
deploy_lambda() {
  ...
  if [[ -z "$existing_function" ]]; then
    ...
    aws lambda create-function \
      ...
      --description "PolizaLab Clients CRUD handler (${ENV})" \
      --environment "Variables={CLIENTS_TABLE=${DYNAMO_TABLE_NAME},POLICIES_TABLE=Policies,TENANT_ID=default,ALLOWED_ORIGIN=https://crm.antesdefirmar.org,LOG_LEVEL=INFO}" \
      --tags "Project=PolizaLab,Feature=Clients,Environment=${ENV}" \
      --region "$REGION"
...
# ---------------------------------------------------------------------------
upsert_api_route() {
  local route_key="$1"       # e.g.  "GET /clients"
  local operation_name="$2"  # e.g.  "ListClients"
  ...
  if [[ -z "$integration_id" || "$integration_id" == "None" ]]; then
    ...
    integration_id=$(aws apigatewayv2 create-integration \
      ...
      --description "client-handler Lambda proxy (${ENV})" \
      --region "$REGION" \
      --query "IntegrationId" \
      --output text)
...
deploy_api_routes() {
  ...
  upsert_api_route "GET /clients"                              "ListClients"        "$authorizer_id"
  upsert_api_route "GET /clients/check-duplicate"             "CheckDuplicateClient" "$authorizer_id"
  upsert_api_route "GET /clients/{clientId}"                  "GetClient"          "$authorizer_id"
  upsert_api_route "POST /clients"                            "CreateClient"       "$authorizer_id"
  upsert_api_route "POST /clients/upsert"                     "UpsertClient"       "$authorizer_id"
  upsert_api_route "PATCH /clients/{clientId}"                "UpdateClient"       "$authorizer_id"
  upsert_api_route "POST /clients/{clientId}/archive"         "ArchiveClient"      "$authorizer_id"
  upsert_api_route "POST /clients/{clientId}/unarchive"       "UnarchiveClient"    "$authorizer_id"
  upsert_api_route "POST /clients/{clientId}/policies/{policyId}" "LinkPolicyToClient" "$authorizer_id"
  ...
  aws apigatewayv2 create-deployment \
    --api-id "$API_ID" \
    --stage-name '$default' \
    --description "Clients feature deployment - $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    --region "$REGION" \
    --output text > /dev/null
...
# ===========================================================================
log "======================================================"
log "  PolizaLab CRM — Clients Feature Deployment"
log "  Environment : $ENV"
log "  Region      : $REGION"
log "  Account     : $ACCOUNT_ID"
log "  API Gateway : $API_ID"
log "  Lambda      : $FUNCTION_NAME"
log "  DynamoDB    : $DYNAMO_TABLE_NAME"
log "======================================================"
...
log ""
log "  Endpoint base:"
log "    https://${API_ID}.execute-api.${REGION}.amazonaws.com/clients"
log "======================================================"
