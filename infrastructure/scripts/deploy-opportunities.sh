#!/usr/bin/env bash
# =============================================================================
# deploy-opportunities.sh — PolizaLab CRM | Opportunities feature deployment
#
# Usage:
#   ./deploy-opportunities.sh [--env dev|stage|prod] [--region us-east-1]
#
# What this script does:
#   1. Creates/updates the Opportunities DynamoDB table via CloudFormation
#   2. Creates/updates the IAM role + inline policy via CloudFormation
#   3. Packages and deploys the opportunity-handler Lambda function
...
#   - Lambda source code at: lambda/opportunity-handler/handler.py
# =============================================================================
set -euo pipefail
...
LAMBDA_SRC_DIR="${PROJECT_ROOT}/lambda/opportunity-handler"
...
# Derived names
# ---------------------------------------------------------------------------
FUNCTION_NAME="polizalab-opportunity-handler-${ENV}"
DYNAMO_STACK="polizalab-opportunities-table-${ENV}"
IAM_STACK="polizalab-opportunity-handler-role-${ENV}"
ARTIFACTS_BUCKET="polizalab-lambda-artifacts-${ENV}"
ARTIFACTS_KEY="opportunity-handler/${FUNCTION_NAME}.zip"
OPPORTUNITIES_TABLE_NAME="Opportunities-${ENV}"
ACTIVITIES_TABLE_NAME="Activities-${ENV}"
SYSTEM_TABLE_NAME="ActivityTypesSystem-${ENV}"
ROLE_NAME="polizalab-opportunity-handler-role-${ENV}"

DYNAMO_CF="${INFRA_DIR}/dynamodb/opportunities-table.json"
IAM_CF="${INFRA_DIR}/iam/opportunity-handler-policy.json"

TAG_LIST="Project=PolizaLab Feature=Opportunities Environment=${ENV}"
...
upload_lambda_zip() {
  ...
  aws s3 cp "$zip_path" "s3://${ARTIFACTS_BUCKET}/${ARTIFACTS_KEY}" \
    --region "$REGION" \
    --metadata "Environment=${ENV},Feature=Opportunities,Project=PolizaLab"
...
deploy_lambda() {
  ...
  local env_vars="Variables={OPPORTUNITIES_TABLE=${OPPORTUNITIES_TABLE_NAME},ACTIVITIES_TABLE=${ACTIVITIES_TABLE_NAME},SYSTEM_TABLE=${SYSTEM_TABLE_NAME},TENANT_ID=default,ALLOWED_ORIGIN=https://crm.antesdefirmar.org,LOG_LEVEL=INFO}"
  ...
  if [[ -z "$existing_function" ]]; then
    ...
    aws lambda create-function \
      ...
      --description "PolizaLab Opportunities CRUD handler (${ENV})" \
      --environment "$env_vars" \
      --tags "Project=PolizaLab,Feature=Opportunities,Environment=${ENV}" \
      --region "$REGION"
...
# ---------------------------------------------------------------------------
upsert_api_route() {
  local route_key="$1"       # e.g.  "GET /opportunities"
  local operation_name="$2"  # e.g.  "ListOpportunities"
  ...
  if [[ -z "$integration_id" || "$integration_id" == "None" ]]; then
    ...
    integration_id=$(aws apigatewayv2 create-integration \
      ...
      --description "opportunity-handler Lambda proxy (${ENV})" \
      --region "$REGION" \
      --query "IntegrationId" \
      --output text)
    ...
    local statement_id="apigw-${API_ID}-${ENV}-oppty-invoke"
...
deploy_api_routes() {
  ...
  upsert_api_route "GET /opportunities"                                    "ListOpportunities"       "$authorizer_id"
  upsert_api_route "POST /opportunities"                                   "CreateOpportunity"       "$authorizer_id"
  upsert_api_route "GET /opportunities/{opportunityId}"                    "GetOpportunity"          "$authorizer_id"
  upsert_api_route "PATCH /opportunities/{opportunityId}"                  "UpdateOpportunity"       "$authorizer_id"
  upsert_api_route "DELETE /opportunities/{opportunityId}"                 "DeleteOpportunity"       "$authorizer_id"
  upsert_api_route "POST /opportunities/{opportunityId}/advance"           "AdvanceOpportunity"      "$authorizer_id"
  upsert_api_route "POST /opportunities/{opportunityId}/close-won"         "CloseWonOpportunity"     "$authorizer_id"
  upsert_api_route "POST /opportunities/{opportunityId}/close-lost"        "CloseLostOpportunity"    "$authorizer_id"
  upsert_api_route "POST /opportunities/{opportunityId}/quotes"            "AddQuoteToOpportunity"   "$authorizer_id"
  ...
  aws apigatewayv2 create-deployment \
    --api-id "$API_ID" \
    --stage-name '$default' \
    --description "Opportunities feature deployment - $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    --region "$REGION" \
    --output text > /dev/null
...
# ===========================================================================
log "======================================================"
log "  PolizaLab CRM — Opportunities Feature Deployment"
log "  Environment : $ENV"
log "  Region      : $REGION"
log "  Account     : $ACCOUNT_ID"
log "  API Gateway : $API_ID"
log "  Lambda      : $FUNCTION_NAME"
log "  DynamoDB    : $OPPORTUNITIES_TABLE_NAME"
log "======================================================"
...
log ""
log "  Endpoint base:"
log "    https://${API_ID}.execute-api.${REGION}.amazonaws.com/opportunities"
log "======================================================"
