#!/usr/bin/env bash
# =============================================================================
# deploy-activity-types.sh — PolizaLab CRM | Activity Types feature deployment
#
# Usage:
#   ./deploy-activity-types.sh [--env dev|stage|prod] [--region us-east-1]
#
# What this script does:
#   1. Creates/updates the ActivityTypesSystem DynamoDB table via CloudFormation
#      and the ActivityTypesTenant DynamoDB table via CloudFormation
#   2. Creates/updates the IAM role + inline policy via CloudFormation
#   3. Packages and deploys the activity-type-handler Lambda function
...
#   - Lambda source code at: lambda/activity-type-handler/handler.py
# =============================================================================
set -euo pipefail
...
LAMBDA_SRC_DIR="${PROJECT_ROOT}/lambda/activity-type-handler"
...
# Derived names
# ---------------------------------------------------------------------------
FUNCTION_NAME="polizalab-activity-type-handler-${ENV}"
SYSTEM_TABLE_STACK="polizalab-activity-types-system-table-${ENV}"
TENANT_TABLE_STACK="polizalab-activity-types-tenant-table-${ENV}"
IAM_STACK="polizalab-activity-type-handler-role-${ENV}"
ARTIFACTS_BUCKET="polizalab-lambda-artifacts-${ENV}"
ARTIFACTS_KEY="activity-type-handler/${FUNCTION_NAME}.zip"
SYSTEM_TABLE_NAME="ActivityTypesSystem-${ENV}"
TENANT_TABLE_NAME="ActivityTypesTenant-${ENV}"
ROLE_NAME="polizalab-activity-type-handler-role-${ENV}"

SYSTEM_TABLE_CF="${INFRA_DIR}/dynamodb/activity-types-system-table.json"
TENANT_TABLE_CF="${INFRA_DIR}/dynamodb/activity-types-tenant-table.json"
IAM_CF="${INFRA_DIR}/iam/activity-type-handler-policy.json"

TAG_LIST="Project=PolizaLab Feature=ActivityTypes Environment=${ENV}"
...
upload_lambda_zip() {
  ...
  aws s3 cp "$zip_path" "s3://${ARTIFACTS_BUCKET}/${ARTIFACTS_KEY}" \
    --region "$REGION" \
    --metadata "Environment=${ENV},Feature=ActivityTypes,Project=PolizaLab"
...
deploy_lambda() {
  ...
  if [[ -z "$existing_function" ]]; then
    ...
    aws lambda create-function \
      ...
      --description "PolizaLab Activity Types catalog handler (${ENV})" \
      --environment "Variables={SYSTEM_TABLE=${SYSTEM_TABLE_NAME},TENANT_TABLE=${TENANT_TABLE_NAME},TENANT_ID=default,ALLOWED_ORIGIN=https://crm.antesdefirmar.org,LOG_LEVEL=INFO}" \
      --tags "Project=PolizaLab,Feature=ActivityTypes,Environment=${ENV}" \
      --region "$REGION"
...
# ---------------------------------------------------------------------------
upsert_api_route() {
  local route_key="$1"       # e.g.  "GET /activity-types"
  local operation_name="$2"  # e.g.  "ListActivityTypes"
  ...
  if [[ -z "$integration_id" || "$integration_id" == "None" ]]; then
    ...
    integration_id=$(aws apigatewayv2 create-integration \
      ...
      --description "activity-type-handler Lambda proxy (${ENV})" \
      --region "$REGION" \
      --query "IntegrationId" \
      --output text)
...
deploy_api_routes() {
  ...
  upsert_api_route "GET /activity-types"          "ListActivityTypes"    "$authorizer_id"
  upsert_api_route "PATCH /activity-types/{code}" "UpdateActivityType"   "$authorizer_id"
  upsert_api_route "POST /activity-types/reorder" "ReorderActivityTypes" "$authorizer_id"
  ...
  aws apigatewayv2 create-deployment \
    --api-id "$API_ID" \
    --stage-name '$default' \
    --description "ActivityTypes feature deployment - $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    --region "$REGION" \
    --output text > /dev/null
...
# ===========================================================================
log "======================================================"
log "  PolizaLab CRM — Activity Types Feature Deployment"
log "  Environment  : $ENV"
log "  Region       : $REGION"
log "  Account      : $ACCOUNT_ID"
log "  API Gateway  : $API_ID"
log "  Lambda       : $FUNCTION_NAME"
log "  System Table : $SYSTEM_TABLE_NAME"
log "  Tenant Table : $TENANT_TABLE_NAME"
log "======================================================"

# Step 1 — DynamoDB tables (system + tenant)
log ""
log "STEP 1/5: DynamoDB tables (system catalog + tenant overrides)"
deploy_cfn_stack "$SYSTEM_TABLE_STACK" "$SYSTEM_TABLE_CF"
deploy_cfn_stack "$TENANT_TABLE_STACK" "$TENANT_TABLE_CF"
...
log ""
log "  System table stack : $SYSTEM_TABLE_STACK"
log "  Tenant table stack : $TENANT_TABLE_STACK"
log "  IAM stack          : $IAM_STACK"
log "  Lambda function    : $FUNCTION_NAME"
log "  API ID             : $API_ID"
log ""
log "  Endpoint base:"
log "    https://${API_ID}.execute-api.${REGION}.amazonaws.com/activity-types"
log "======================================================"
