#!/usr/bin/env bash
# =============================================================================
# deploy-leads.sh — PolizaLab CRM | Leads feature deployment
#
# Usage:
#   ./deploy-leads.sh [--env dev|stage|prod] [--region us-east-1]
#
# What this script does:
#   1. Creates/updates the Leads DynamoDB table via CloudFormation
#   2. Creates/updates the IAM role + inline policy via CloudFormation
#   3. Packages and deploys the lead-handler Lambda function
...
#   - Lambda source code at: lambda/lead-handler/handler.py
# =============================================================================
set -euo pipefail
...
LAMBDA_SRC_DIR="${PROJECT_ROOT}/lambda/lead-handler"
...
# Derived names
# ---------------------------------------------------------------------------
FUNCTION_NAME="polizalab-lead-handler-${ENV}"
DYNAMO_STACK="polizalab-leads-table-${ENV}"
IAM_STACK="polizalab-lead-handler-role-${ENV}"
ARTIFACTS_BUCKET="polizalab-lambda-artifacts-${ENV}"
ARTIFACTS_KEY="lead-handler/${FUNCTION_NAME}.zip"
LEADS_TABLE_NAME="Leads-${ENV}"
CLIENTS_TABLE_NAME="Clients-${ENV}"
POLICIES_TABLE_NAME="Policies-${ENV}"
ACTIVITIES_TABLE_NAME="Activities-${ENV}"
SYSTEM_TABLE_NAME="ActivityTypesSystem-${ENV}"
ROLE_NAME="polizalab-lead-handler-role-${ENV}"

DYNAMO_CF="${INFRA_DIR}/dynamodb/leads-table.json"
IAM_CF="${INFRA_DIR}/iam/lead-handler-policy.json"

TAG_LIST="Project=PolizaLab Feature=Leads Environment=${ENV}"
...
upload_lambda_zip() {
  ...
  aws s3 cp "$zip_path" "s3://${ARTIFACTS_BUCKET}/${ARTIFACTS_KEY}" \
    --region "$REGION" \
    --metadata "Environment=${ENV},Feature=Leads,Project=PolizaLab"
...
deploy_lambda() {
  ...
  if [[ -z "$existing_function" ]]; then
    ...
    aws lambda create-function \
      ...
      --description "PolizaLab Leads CRUD handler (${ENV})" \
      --environment "Variables={LEADS_TABLE=${LEADS_TABLE_NAME},CLIENTS_TABLE=${CLIENTS_TABLE_NAME},POLICIES_TABLE=${POLICIES_TABLE_NAME},ACTIVITIES_TABLE=${ACTIVITIES_TABLE_NAME},SYSTEM_TABLE=${SYSTEM_TABLE_NAME},TENANT_ID=default,ALLOWED_ORIGIN=https://crm.antesdefirmar.org,LOG_LEVEL=INFO}" \
      --tags "Project=PolizaLab,Feature=Leads,Environment=${ENV}" \
      --region "$REGION"
...
# ---------------------------------------------------------------------------
upsert_api_route() {
  local route_key="$1"       # e.g.  "GET /leads"
  local operation_name="$2"  # e.g.  "ListLeads"
  ...
  if [[ -z "$integration_id" || "$integration_id" == "None" ]]; then
    ...
    integration_id=$(aws apigatewayv2 create-integration \
      ...
      --description "lead-handler Lambda proxy (${ENV})" \
      --region "$REGION" \
      --query "IntegrationId" \
      --output text)
...
deploy_api_routes() {
  ...
  upsert_api_route "GET /leads"                        "ListLeads"       "$authorizer_id"
  upsert_api_route "GET /leads/{leadId}"               "GetLead"         "$authorizer_id"
  upsert_api_route "POST /leads"                       "CreateLead"      "$authorizer_id"
  upsert_api_route "PATCH /leads/{leadId}"             "UpdateLead"      "$authorizer_id"
  upsert_api_route "POST /leads/{leadId}/log-contact"  "LogLeadContact"  "$authorizer_id"
  upsert_api_route "POST /leads/{leadId}/convert"      "ConvertLead"     "$authorizer_id"
  upsert_api_route "DELETE /leads/{leadId}"            "DeleteLead"      "$authorizer_id"
  ...
  aws apigatewayv2 create-deployment \
    --api-id "$API_ID" \
    --stage-name '$default' \
    --description "Leads feature deployment - $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    --region "$REGION" \
    --output text > /dev/null
...
# ===========================================================================
log "======================================================"
log "  PolizaLab CRM — Leads Feature Deployment"
log "  Environment : $ENV"
log "  Region      : $REGION"
log "  Account     : $ACCOUNT_ID"
log "  API Gateway : $API_ID"
log "  Lambda      : $FUNCTION_NAME"
log "  DynamoDB    : $LEADS_TABLE_NAME"
log "======================================================"
...
log ""
log "  Endpoint base:"
log "    https://${API_ID}.execute-api.${REGION}.amazonaws.com/leads"
log "======================================================"
